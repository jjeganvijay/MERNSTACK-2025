...
export async function runSpinnerTask(display, start, stop, task) {
    display.spinner.start(`${start}...`);
    const result = await tryCatchError(task());
    if (result instanceof Error) {
        display.spinner.stop(`Error ${start[0].toLowerCase()}${start.slice(1)}:`, 1);
...
//# sourceMappingURL=runSpinnerTask.js.map