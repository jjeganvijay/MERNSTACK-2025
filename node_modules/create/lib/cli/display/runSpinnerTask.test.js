...
describe("runSpinnerTask", () => {
    it("displays the error when the task throws an error", async () => {
        ...
        const actual = await runSpinnerTask(createClackDisplay(), "Running task", "Ran task", vi.fn().mockRejectedValueOnce(error));
    ...
    it("displays a general log when the task resolves with a value", async () => {
        ...
        const actual = await runSpinnerTask(createClackDisplay(), "Running task", "Ran task", vi.fn().mockResolvedValueOnce(expected));
...
//# sourceMappingURL=runSpinnerTask.test.js.map